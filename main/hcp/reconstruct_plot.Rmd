...
ggplot(data=plot.df,aes(x=log(m),y=mse,color=dataset)) + geom_smooth(se=FALSE) +
  ...
  scale_y_continuous(breaks = c(0.03, 0.04, min(res_table$`test mse`), 0.05, 0.06, 0.07, 0.08), 
                     labels = c(0.03, 0.04, round(min(res_table$`test mse`),4), 0.05, 0.06, 0.07, 0.08)) + labs(title = "MSE of LKFLAG in the neuroimaging reconstruction with 917 subjects", y = "MSE", x = "The logarithm of the number of training samples")
