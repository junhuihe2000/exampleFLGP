...
```{r}
output_path = "plots/kernel density"
```
...
```{r}
p = ggplot() + geom_path(aes(theta,y_hk), color="red", lwd=0.75) +
...
for (K in Ks) {
  ...
  # plot
  p_flgp = p + geom_path(aes(theta,y_fgl), color="steelblue", lwd=0.75, linetype="solid")
  ggsave(filename=file.path(output_path, paste0("K=",K,"_revision.pdf")), p_flgp, width=5, height=5)
}
```
